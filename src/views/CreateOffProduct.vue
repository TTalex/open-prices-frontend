        ...
        >
          ...
          <v-card-text>
            ...
            <div v-if="!productExists">
              ...
              <i>To add a product image, enable draw mode below proof picture and draw a rectangle around the product.</i>
...
export default {
  ...
  methods: {
      ...
      return api.getPrices({product_code: this.productForm.product_code, order_by: constants.PRICE_ORDER_LIST[2].key })
        .then((data) => {
          ...
          if (this.priceList.length) {
            ...
            this.setShownProof()
            this.loadPriceTags(lastPrice.id)
          } else {
            this.productForm = {
              ...this.productForm,
              product_name: null,
    ...
    getMissingProductsWithPrices() {
      return api.getProducts({ price_count__gte: 1, source_isnull: true, order_by: '-price_count' })
        .then((data) => {
    ...
    setShownProof() {
      ...
      this.proofImageSrc = `${import.meta.env.VITE_OPEN_PRICES_APP_URL}/img/${this.shownProof.file_path}`
      this.zoomLevel = 1
